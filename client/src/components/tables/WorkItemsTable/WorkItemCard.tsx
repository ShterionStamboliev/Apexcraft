...
const WorkItemCard = ({ workItems }: WorkItemCardProps) => {
    return (
        <>
            {workItems.pages.map((page) =>
                page.map((item) => (
                    <Card
                        className='w-full sm:w-full md:w-full lg:max-w-[24rem] shadow-md shadow-slate-700/20 transition duration-300 ease-in-out hover:shadow-md dark:hover:shadow-slate-700/40'
                        key={item.id}
                    >
                        ...
                        <CardContent className='p-6'>
                            <div className='flex flex-col lg:flex-row gap-2 justify-between text-sm text-muted-foreground'>
                                <div className=''>
                                    ...
                                    <div>
                                        End:{' '}
                                        {format(new Date(item.end_date!), 'PP')}
                                    </div>
                                </div>
